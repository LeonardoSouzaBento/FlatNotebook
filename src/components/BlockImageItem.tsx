...
import { BlockImage, ImageEdit } from "@/types/document";
import { MoreVertical, Crop, Maximize2, Trash2 } from "lucide-react";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
const BlockImageItem: React.FC<BlockImageItemProps> = ({ image, onUpdate, onDelete }) => {
      ...
      <div className="relative group my-2 inline-block">
        ...
        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
          <Popover open={menuOpen} onOpenChange={setMenuOpen}>
            ...
            <PopoverContent className="w-auto p-1 flex gap-1" side="left" align="start">
              <button
                onClick={() => { setMenuOpen(false); setCropOpen(true); }}
              ...
              <button
                onClick={() => { setMenuOpen(false); onDelete(image.id); }}
      ...
      <Dialog open={cropOpen} onOpenChange={setCropOpen}>
        <DialogContent>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label>X (%)</Label>
              <Input type="number" min={0} max={100} value={cropValues.x} onChange={(e) => setCropValues((v) => ({ ...v, x: Number(e.target.value) }))} />
            </div>
            <div>
              <Label>Y (%)</Label>
              <Input type="number" min={0} max={100} value={cropValues.y} onChange={(e) => setCropValues((v) => ({ ...v, y: Number(e.target.value) }))} />
            </div>
            <div>
              <Label>Largura (%)</Label>
              <Input type="number" min={1} max={100} value={cropValues.width} onChange={(e) => setCropValues((v) => ({ ...v, width: Number(e.target.value) }))} />
            </div>
            <div>
              <Label>Altura (%)</Label>
              <Input type="number" min={1} max={100} value={cropValues.height} onChange={(e) => setCropValues((v) => ({ ...v, height: Number(e.target.value) }))} />
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={resizeOpen} onOpenChange={setResizeOpen}>
        <DialogContent>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label>Largura (%)</Label>
              <Input type="number" min={10} max={200} value={resizeValues.width} onChange={(e) => setResizeValues((v) => ({ ...v, width: Number(e.target.value) }))} />
            </div>
            <div>
              <Label>Altura (%)</Label>
              <Input type="number" min={10} max={200} value={resizeValues.height} onChange={(e) => setResizeValues((v) => ({ ...v, height: Number(e.target.value) }))} />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setResizeOpen(false)}>Cancelar</Button>
